...
# Define the Article Analyst Agent
article_analyst = Agent(
    role="Article Analyst",
    goal="Extract factual statements from scientific articles that require verification",
    backstory="You are a meticulous researcher specializing in scientific text analysis.",
    verbose=True,
    llm=OllamaLLM(),
...
# Define the task
def create_analysis_task(article_text: str) -> Task:
    return Task(
        description=f"{PROMPT}\n\nArticle:\n{article_text}",
        agent=article_analyst,
        expected_output="A list of extracted statements from the article that require fact-checking",
    )

# Crew definition
def create_crew(article_text: str) -> Crew:
    return Crew(
        agents=[article_analyst],
        tasks=[create_analysis_task(article_text)],
        verbose=0,
        telemetry=False
    )

# Process the article and extract chunks as a list
def process_article(article_text: str) -> list[str]:
    crew = create_crew(article_text)
    result = crew.kickoff()
    ...
    if isinstance(result, str):
        for line in result.splitlines():
            ...
            if cleaned_line:
                # Remove leading numbering (e.g., "1. ", "2. ") if present
                cleaned_line = re.sub(r'^\d+\.\s+', '', cleaned_line)
                chunks.append(cleaned_line)
    return chunks  # Returns ["chunk1", "chunk2", ...]

@app.post("/analyze-article/")
async def analyze_article(request: ArticleRequest):
    try:
        chunks = process_article(request.article)
        return {"extracted_chunks": chunks}  # Returns JSON: {"extracted_chunks": ["chunk1", "chunk2", ...]}
...
The analysis method we use, based on QIM, represents a step toward future experimental studies on vibrational decoherence assisted by quantum information measures. Although out of reach at present owing to the lack of suitable experimental measures of quantum correlations, such studies are anticipated to advance the field of quantum information science in molecular systems (62–66)."""
    ...
    chunks = process_article(ARTICLE_TEXT)
    print("Extracted Chunks as List:")
    print(chunks)  # Directly print the list to show ["chunk1", "chunk2", ...]
